...
const PeminjamanTab = () => {
    ...
    return (
        <div className="w-full grid grid-cols-5 place-items-center gap-x-2 rounded-xl border">
            {tabsMenu.map(item => {
                return (
                <div key={item.id} onClick={() => handleTabs({id: item.id, filter: item.filter})} className={`${item.title === filter ? 'bg-muted text-primary-foreground' : ''} w-full cursor-default col-span-1 py-4 first:rounded-l last:rounded-l hover:text-primary-foreground duration-300 ease-in-out`}>
