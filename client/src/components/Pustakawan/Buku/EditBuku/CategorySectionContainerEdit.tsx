...
import AddCategoriDialogEdit from './AddCategoriDialogEdit'

const CategorySectionContainerEdit = ({buku} : {buku: any}) => {
...
const dataSelect = isLoading ? ['memuat'] : response.data.map((item: any) => item.nama)
  ...
  const [selectedValue, setSelected] = useState<Array<string>>(buku.kategori)
  ...
  return (
    <section className='w-full p-4 bg-accent/10 rounded-xl'>
      ...
      <AddCategoriDialogEdit />
    </section>
  )
}

export default CategorySectionContainerEdit