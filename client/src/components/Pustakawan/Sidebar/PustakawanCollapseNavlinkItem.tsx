  ...
  return (
    <Accordion type="single" collapsible className="w-full">
      <AccordionItem value="item-1" className="m-0">
        <AccordionTrigger className={`w-full flex items-center justify-between py-3 px-2 group bg-transparent hover:bg-muted rounded hover:no-underline ${isActive && 'bg-primary'}`}>
        ...
        {data.children.map((item: any, index: number) => {
          ...
          return (
            <AccordionContent onClick={() => handleClick(item.url)} key={index} className="flex items-center gap-x-2 text-balance flex-1 ml-6 mt-2 p-2 hover:bg-muted duration-200 ease-in-out rounded" asChild>
