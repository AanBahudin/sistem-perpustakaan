...
const StatusPinjamanFilter = () => {

  const [searchParams] = useSearchParams()

  const {statusPeminjaman} = useSelector((state: any) => state.peminjamanFilterSheetState)
  const initialParams = searchParams.get('statusPeminjaman') || statusPeminjaman

  const value = ['Semua', 'Dipinjam', 'Dikembalikan', 'Terlambat', 'Diajukan', 'Ditolak']
