...
const PengajuanRatioDougnut = ({pengajuanRatio} : {pengajuanRatio: any}) => {
            ...
            <main className='w-full flex items-center justify-center gap-x-4 mt-4'>
                {label.map((item: string, index: number) => {
                  ...
                  return (
                    ...
                    <Badge style={{backgroundColor: bgColor}} className='text-white'>{pengajuanRatio[index]} {item}</Badge>
                  )
                })}
            </main>
        </section>
...
export default PengajuanRatioDougnut