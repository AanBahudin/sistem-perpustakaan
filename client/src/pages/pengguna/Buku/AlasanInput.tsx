...
const AlasanInput = ({defaultAlasan, perpanjangan, type='peminjaman'} : AlasanInputType) => {
  ...
  return (
    <section className="w-full flex flex-col gap-y-2">
        ...
        <Textarea disabled={!isObjectEmpty} name="alasan" id="alasan" defaultValue={defaultAlasan} maxLength={250} onChange={e => store.dispatch(setAlasan(e.target.value))} required placeholder={`Alasan ${type}`} />
    </section>
  )
}
